...
import { PackageDemo1Service, UserModel } from "../api";
import { UUIButtonElement } from "@umbraco-cms/backoffice/external/uui";
import { UMB_NOTIFICATION_CONTEXT, UmbNotificationContext } from "@umbraco-cms/backoffice/notification";
import { UMB_CURRENT_USER_CONTEXT, UmbCurrentUserModel } from "@umbraco-cms/backoffice/current-user";

const elementName = 'pack-1-dashbord';
@customElement(elementName)
export class Pack1DashboardElement extends UmbElementMixin(LitElement) {
  ...
  #onClickWhatsTheTimeMrWolf = async (ev: Event) => {
    ...
    const { data, error } = await PackageDemo1Service.whatsTheTimeMrWolf();
  ...
  #onClickWhatsMyName = async (ev: Event) => {
    ...
    const { data, error } = await PackageDemo1Service.whatsMyName();
...
export default Pack1DashboardElement;

declare global {
  interface HTMLElementTagNameMap {
    [elementName]: Pack1DashboardElement;
  }
}
