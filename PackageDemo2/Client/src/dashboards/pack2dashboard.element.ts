...
import { PackageDemo2Service, UserModel } from "../api";
import { UUIButtonElement } from "@umbraco-cms/backoffice/external/uui";
import { UMB_NOTIFICATION_CONTEXT, UmbNotificationContext } from "@umbraco-cms/backoffice/notification";
import { UMB_CURRENT_USER_CONTEXT, UmbCurrentUserModel } from "@umbraco-cms/backoffice/current-user";

const elementName = 'pack-2-dashbord';
@customElement(elementName)
export class Pack2DashboardElement extends UmbElementMixin(LitElement) {
  ...
  #onClickWhoAmI = async (ev: Event) => {
    ...
    const { data, error } = await PackageDemo2Service.whoAmI();
    ...
    if (this.#notificationContext) {
      this.#notificationContext.peek("warning", {
        data: {
          headline: `Dashboard 2: You are ${this._serverUserData?.name}`,
          message: `Dashboard 2: Your email is ${this._serverUserData?.email}`,
        }
      })
    }
  }
  ...
  #onClickWhatsTheTimeMrWolf = async (ev: Event) => {
    ...
    const { data, error } = await PackageDemo2Service.whatsTheTimeMrWolf();
  ...
  #onClickWhatsMyName = async (ev: Event) => {
    ...
    const { data, error } = await PackageDemo2Service.whatsMyName();
...
export default Pack2DashboardElement;

declare global {
  interface HTMLElementTagNameMap {
    [elementName]: Pack2DashboardElement;
  }
}
